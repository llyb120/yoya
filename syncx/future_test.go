...
func TestAsyncBasic(t *testing.T) {
	...
	future := Async[int](noArgFn, 5*time.Second)()
	...
	future = Async[int](addFn, 5*time.Second)(10, 20)
	result, err = future.Get()
	if err != nil {
		t.Errorf("期望无错误，但得到: %v", err)
	}
...
func TestAsyncLongRunning(t *testing.T) {
	...
	future := Async[int](slowFn, 5*time.Second)()
	result, err := future.Get()
	if err != nil {
		t.Errorf("期望无错误，但得到: %v", err)
	}
...
func TestAsyncWithError(t *testing.T) {
	...
	future := Async[Result](errorFn, 5*time.Second)()
	result, err := future.Get()
	if err != nil {
		t.Errorf("期望无错误，但得到: %v", err)
	}
